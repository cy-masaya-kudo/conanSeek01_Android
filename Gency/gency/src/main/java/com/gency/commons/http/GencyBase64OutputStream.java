...
public class GencyBase64OutputStream extends FilterOutputStream {
    private final GencyBase64.Coder coder;
     ...
     * @param flags bit flags for controlling the encoder; see the constants in {@link GencyBase64}
     */
    public GencyBase64OutputStream(OutputStream out, int flags) {
        this(out, flags, true);
    }
     ...
     */
    public GencyBase64OutputStream(OutputStream out, int flags, boolean encode) {
        super(out);
        this.flags = flags;
        if (encode) {
            coder = new GencyBase64.Encoder(flags, null);
        } else {
            coder = new GencyBase64.Decoder(flags, null);
        }
    }
    ...
    public void close() throws IOException {
        ...
        try {
            if ((flags & GencyBase64.NO_CLOSE) == 0) {
                out.close();
            } else {
                out.flush();
            }
    ...
    private void internalWrite(byte[] b, int off, int len, boolean finish) throws IOException {
        ...
        if (!coder.process(b, off, len, finish)) {
            throw new GencyBase64DataException("bad base-64");
        }
